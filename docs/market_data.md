...
只有到停牌开始后才对策略可见。完整解析逗号分隔的全部时段，小时允许一位或两位，秒可省略；
各段按 `[开始, 结束)` 判断，任一段仍处于停牌中就返回 `True`，段间空隙和全部结束后返回
`False`。未来尚未开始的段不提前改变状态。清洗与查询共用同一个解析函数，任何坏片段都是
源数据错误，不能只读取合法前缀或静默返回未知状态。
