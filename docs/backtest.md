...
核销同时清理该证券尚未上市的红股事件，原上市日不会再恢复它们；已确认的现金应收继续按派息日结算。
`metrics.json` 的 `corporate_action_scope` 记录口径。吸收合并、换股、配股、现金选择权和
退市后回收尚未实现，分红源也不能保证发现这些事件；结果不能解释为完整经济结算。
...
最低量和递增单位约束原始委托；合法订单受成交容量或现金限制时可以逐股部分成交，
即使成交量低于最低申报量也正常记账。
...
不会覆盖已经可见的更新价格。QMT 历史与实时标签默认均为结束时刻，
可通过 DataReader 的 `qmt_history_time_label` / `qmt_realtime_time_label` 显式指定。
模式和时间标签都会写入运行元数据。推送默认值的社区依据、跨接口推断和实测边界见
...
- 下一根 Bar 开盘价一次性撮合；
- 停牌不成交，涨停不买，跌停不卖；
- 申报数量遵守对应板块规则，清仓允许零股；合法委托可逐股部分成交；
