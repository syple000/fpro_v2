...
连接超时和临时断线默认最多重试 3 次，等待时间依次为 1、2、4 秒；每次重试仍重新经过同一个
限流器。字段错误、权限错误和接口参数错误不会重试。
...
调用方传入的日期区间就是本次实际刷新区间，不另外维护“请求过一次便永久跳过”的覆盖元数据。
因此调用方可以在日常任务中主动向前多传几天：行情使用较短回看窗口，财报、分红等使用较长
窗口。同一区间再次执行会重新请求；有返回的完整全市场日期截面直接替换对应 Parquet 分区，
所以不会因为重拉而堆积重复行情。接口返回空数据时不清空已有分区，下次滚动刷新还会继续请求。
...
第二次会直接刷新整个 `2017-01-01` 至 `2026-08-19`，包括与第一次重叠的 2021--2022 年。
重叠日期使用新截面替换旧分区，不会重复落盘。这是用少量重复请求换取简单、可预测的同步逻辑；
日常增量任务无需每次传全历史，只需按上面的建议传滚动回看窗口。
